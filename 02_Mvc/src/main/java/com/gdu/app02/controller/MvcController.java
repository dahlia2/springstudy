...
public class MvcController {
	...
	*/
	...
	/*
		 ...
		 2. Model2(Jsp/Servlet)에서는 HttpServletRequest request 객체를 사용해서 데이터를 전달하지만, (요청과 응답 모두 request가 했음)
		 	스프링에서는 Model model 객체를 사용한다. (forward을 위해서 model 사용)
	...
	*/
	...
	/*
		...
		public String detailMe(String name,     // 요청 파라미터 name이 없으면 null로 저장된다.
							   int age,			// 요청 파라미터 age가 없으면 null을 int로 변환하려고 하기 때문에 오류가 발생한다. => String으로 변환하면 됨. (null로 저장)
							   Model model) {
	...
	*/
	...
	/*
	  3. 커맨드 객체를 이용한 요청 파라미터 처리
	  	1) 파라미터를 필드로 가지고 있는 객체를 커맨드 객체라고 한다.
	  	2) setter가 사용된다.
	  	3) 커맨드 객체는 자동으로 Model에 저장된다.
	  
	*/
	@GetMapping("/detail.gdu")
	public String detail(Person p) {  // name과 age를 필드로 가진 커맨드 객체 Person p
									  // Model에 저장될 때 객체 이름인 p를 사용하지 않고,
									  // 객체 타입인 Person을 사용한다.
									  // Model에 저장되는 속성명은 객체 타입 Person을 person으로 수정해서 사용한다. (첫 글자를 소문자로)
		return "mvc/detail";
	}

	/*
	@GetMapping("/detail.gdu")
	public String detail(@ModelAttribute(value="p") Person person) {   // Model에 저장할 속성명을 p로 해 주세요.
																	   // -> jsp문서에서 ${p.name} ${p.age}와 같은 형식으로 확인할 수 있다.
	*/
